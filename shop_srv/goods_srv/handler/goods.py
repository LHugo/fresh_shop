...
class GoodsServicer(goods_pb2_grpc.GoodsServicer):
    def convert_model_to_message(self, goods):
        ...
        info_rsp.transportFree = goods.transport_free
    ...
    def CreateGoods(self, request: goods_pb2.GoodInfoRequest, context):
        ...
        goods.transport_free = request.transportFree
    ...
    @logger.catch
    def GetAllCategorysList(self, request: empty_pb2, context):
        # 商品的分类
        level1 = []
        level2 = []
        level3 = []
        ...
        for data3 in level3:
            for data2 in level2:
                if data3["parent"] == data2["id"]:
                    if "sub_categorys" not in data2:
                        data2["sub_categorys"] = [data3]
                    else:
                        data2["sub_categorys"].append(data3)

        for data2 in level2:
            for data1 in level1:
                if data2["parent"] == data1["id"]:
                    if "sub_categorys" not in data1:
                        data1["sub_categorys"] = [data2]
                    else:
                        data1["sub_categorys"].append(data2)
    ...
    def CategoryBrandList(self, request: goods_pb2.CategoryBrandFilterRequest, context):
        ...
        for category_brand in category_brands:
            ...
            category_brand_rsp.brand.id = category_brand.brand_id.id
            category_brand_rsp.brand.name = category_brand.brand_id.name
            category_brand_rsp.brand.logo = category_brand.brand_id.logo

            category_brand_rsp.category.id = category_brand.category_id.id
            category_brand_rsp.category.name = category_brand.category_id.name
            category_brand_rsp.category.parentCategory = category_brand.category_id.parent_category_id
            category_brand_rsp.category.level = category_brand.category_id.level
            category_brand_rsp.category.isTab = category_brand.category_id.is_tab
    ...
    def GetCategoryBrandList(self, request, context):
        ...
        try:
            ...
            category_brands = GoodsCategoryBrand.select().where(GoodsCategoryBrand.category_id == category)
            rsp.total = category_brands.count()
            for category_brand in category_brands:
                brand_rsp = goods_pb2.BrandInfoResponse()
                brand_rsp.id = category_brand.brand_id.id
                brand_rsp.name = category_brand.brand_id.name
                brand_rsp.logo = category_brand.brand_id.logo
    ...
    def CreateCategoryBrand(self, request: goods_pb2.CategoryBrandRequest, context):
        ...
        try:
            ...
            category_brand.brand_id = brand
            category = Category.get(request.categoryId)
            category_brand.category_id = category
            category_brand.save()
