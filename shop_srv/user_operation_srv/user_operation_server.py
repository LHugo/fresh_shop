...
from user_operation_srv.proto import message_pb2_grpc, address_pb2_grpc, favourite_pb2_grpc
from user_operation_srv.handler.message import MessageServicer
from user_operation_srv.handler.favourite import FavouriteServicer
from user_operation_srv.handler.address import AddressServicer
...
def server():
    ...
    if args.port == 0:
        ...
        port = args.port
    logger.add("logs/user_operation_srv_{time}.log")
    # 拦截器实例化
    ...
    favourite_pb2_grpc.add_FavouriteServicer_to_server(FavouriteServicer(), server)
    # 3. 注册健康检查
