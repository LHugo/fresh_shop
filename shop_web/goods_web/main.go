...
import (
	...
	"shop_web/goods_web/initialize"
	"shop_web/goods_web/utils"
	"shop_web/goods_web/utils/register/consul"
	"syscall"
)
...
func main() {
	...
	if err := registerClient.Register(global.ServerConfig.Host, global.ServerConfig.Port, global.ServerConfig.Name, global.ServerConfig.Tags, serviceId); err != nil{
		zap.S().Panic("商品服务注册失败", err.Error())
	}
	...
	if err := registerClient.DeRegister(serviceId); err != nil{
		zap.S().Panic("注销商品服务失败：", err.Error())
	}else {
		zap.S().Infof("注销商品服务成功")
	}
